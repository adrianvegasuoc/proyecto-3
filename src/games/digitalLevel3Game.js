...
const HEADLINES_L3 = [
  ...
  {
    text: "Un vídeo demuestra que el 5G causa enfermedades, según ‘un científico’ anónimo.",
    isReliable: false,
    explanation:
      "Fuente anónima + afirmación extraordinaria sin evidencia: patrón clásico de desinformación.",
  },
  {
    text: "Una filtración revela que todos los móviles escuchan conversaciones incluso apagados.",
    isReliable: false,
    explanation:
      "Afirmación extrema y difícil de sostener técnicamente; típico rumor viral sin pruebas.",
  },
  {
    text: "Una organización pública lanza una campaña para enseñar a detectar bulos en redes sociales.",
    isReliable: true,
    explanation:
      "Campañas de alfabetización mediática existen y son habituales.",
  },
  {
    text: "Un estudio revisado por pares explica cómo el clickbait aumenta la desinformación.",
    isReliable: true,
    explanation:
      "Investigación académica sobre clickbait y desinformación es un tema real y frecuente.",
  },
];
...
const PENALTY_PER_FAIL = 10; // penalización por fallo (-10 puntos)
...
function createInitialState() {
  return {
    headlines: shuffle(HEADLINES_L3),
    currentIndex: 0,
    attempts: 0,
    score: 0,
...
function startTimer(rootElement) {
  ...
  timerId = setInterval(() => {
    ...
    if (gameState.timeLeft <= 0) {
      ...
      endGame("time");
      renderDigitalLevel3(rootElement);
    } else {
...
function endGame(reason) {
  ...
  if (gameState.score >= gameState.targetScore) {
    ...
    // Recompensa: 80 monedas por superar el nivel 3
    addCoins(80);
    completeLevel("digital", 3);
    saveGameState();
  } else {
...
export function startDigitalLevel3(rootElement) {
  gameState = createInitialState();
  startTimer(rootElement);
  renderDigitalLevel3(rootElement);
}

export function stopDigitalLevel3() {
  stopTimer();
  gameState = null;
}

export function restartDigitalLevel3(rootElement) {
  gameState = createInitialState();
  startTimer(rootElement);
  renderDigitalLevel3(rootElement);
}

export function handleDigitalLevel3Choice(choice, rootElement) {
  if (!gameState || gameState.finished) return;
  ...
  if (isCorrect) {
    ...
    // Penalización
    gameState.score -= PENALTY_PER_FAIL;
...
export function renderDigitalLevel3(rootElement) {
  ...
  `;
    ...
    if (gameState.success) {
      mainMessage = "¡Nivel 3 superado!";
      subMessage = `Has conseguido ${gameState.score} puntos en ${gameState.attempts} intentos. Recompensa: 80 monedas.`;
    } else {
